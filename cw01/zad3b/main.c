...
void fill_array(WrappedArray *array){
    for(int i=0; i < array->height; i++){
        ...
        #ifdef DLL
        void (*create_block)(WrappedArray*,char*,int) = dlsym(dll, "create_block");
        #endif
...
void remove_number(WrappedArray* array, int n){
    for(int i=0; i < n; i++){
        #ifdef DLL
        void (*delete_block)(WrappedArray*,int) = dlsym(dll, "delete_block");
        #endif
...
void exec_operation(WrappedArray* array, char* operation, int number){
    if(strcmp(operation,"search")==0){
        #ifdef DLL
        char* (*find_block)(WrappedArray*,int) = dlsym(dll, "find_block");
        #endif
...
int main(int argc, char**argv){
    ...
    #ifdef DLL
    WrappedArray* (*create_array)(int,int,int) = dlsym(dll, "create_array");
    #endif
